...
{
    public class InsertOrUpdateResult : ICommand
    {
        private readonly int _repairId;

        private readonly IEnumerable<Node> _nodes;
        
        public InsertOrUpdateResult(Node node, int repairId)
            :this(new[] { node }, repairId) {}

        public InsertOrUpdateResult(IEnumerable<Node> nodes, int repairId)
        {
            _nodes = nodes;
            _repairId = repairId;
        }
        ...
        {
            const string sqlInsert = @"INSERT INTO [Repairs]"+
                                        " (Id, Id, ParrentId, Name, Val, TypeVal)" +
                                        " VALUES" +
                                        " ('{0}', '{1}', '{2}', {3}, '{4}', '5');";

            const string sqlUpdate = @"UPDATE [Repairs] Set" +
                                        " [ParrentId] = '{2}'," +
                                        " [Name] = '{3}'," +
                                        " [Val] = {4}," +
                                        " [TypeVal] = '{5}'" +
